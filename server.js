...
async function initDefaults() {
    // 🧹 ОЧИСТКА БАЗЫ - раскомментируйте ТОЛЬКО СЕЙЧАС
...
app.put('/api/settings', async (req, res) => {
  try {
    ...
    let s = await Settings.findOne();
    if (s) { s.mainTitle = mainTitle; s.mainSubtitle = mainSubtitle; s.title = title; s.desc = desc; s.phone = phone; await s.save(); }
    else s = await Settings.create({ mainTitle, mainSubtitle, title, desc, phone, globalBotEnabled: true });
...
// ✅ ИСПРАВЛЕННЫЙ ENDPOINT ОТВЕТА ОПЕРАТОРА (Удаляет "В обработке")
