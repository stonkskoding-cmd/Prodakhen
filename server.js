...
// Настройки (увеличен лимит для фото)
...
const chatSchema = new mongoose.Schema({
  ...
  messages: [{ 
    ...
    text: String, 
    extra: mongoose.Schema.Types.Mixed, // Для хранения данных об анкете/услугах
    time: { type: Date, default: Date.now } 
  }],
  waitingForOperator: { type: Boolean, default: false },
  botEnabled: { type: Boolean, default: true },
  botStep: { type: String, default: 'greet' }, // Этап диалога
...
app.put('/api/settings', async (req, res) => { try { let s = await Settings.findOne(); if(s) Object.assign(s, req.body), await s.save(); res.json({success:true}); } catch(e){res.status(500).json({error:e.message})} });
...
app.post('/api/auth', async (req, res) => { try { const u = await User.findOne(req.body); if(!u) return res.status(401).json({success:false,message:'Неверный логин'}); res.json({success:true,user:{username:u.username,role:u.role}}); } catch(e){res.status(500).json({error:e.message})} });
...
// ✅ БЫСТРЫЙ ЗАКАЗ (Инициализация чата с конкретной девушкой)
app.post('/api/chat/init', async (req, res) => {
  try {
    ...
    if (!chat) {
      ...
      // Если чат уже есть, сбрасываем его под новый заказ
      chat.messages = []; 
    ...
    // Бот сразу приветствует и показывает анкету + услуги
...
app.post('/api/chat/send', async (req, res) => {
  try {
    ...
    // Логика работает только если бот активен и мы не ждем оператора
    if (isBotActive && !chat.waitingForOperator) {
      ...
      if (chat.botStep === 'greet' || chat.botStep === 'asking_city') {
        ...
        if (fc) { 
          ...
          botReply = { text: 'Напишите город (Луганск, Стаханов или Первомайск).', type: 'text' }; 
      ...
      // 3. Выбор услуги
        ...
        if (fs) { 
          ...
          chat.waitingForOperator = true; // Передаем оператору
          chat.botStep = 'waiting'; 
        } else { 
          botReply = { text: 'Напишите услугу (например: Встреча).', type: 'text' }; 
        }
      }
    }
...
// ✅ ОТВЕТ ОПЕРАТОРА (И завершение заказа)
app.post('/api/admin/chat/reply', async (req, res) => { 
  try { 
    ...
    if (chat.messages.length > 0 && chat.messages[chat.messages.length - 1].extra?.type === 'processing') chat.messages.pop(); 
    
    // Пишем сообщение оператора
    chat.messages.push({ type: 'bot', text: `[Оператор] ${text}`, time: new Date() }); 
    
    // ✅ ЗАВЕРШЕНИЕ ЗАКАЗА: Сбрасываем бота в начало, чтобы он мог принять новый заказ
    chat.waitingForOperator = false; 
    chat.botStep = 'greet'; 
